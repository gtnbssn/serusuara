<script lang="ts">
	...
	import { AutoColliders, Collider, RigidBody, useJoint } from '@threlte/rapier';
...
<T.Group {position}>
	<RigidBody />
	<RigidBody enabledTranslations={[false, false, false]} enabledRotations={[false, false, false]}>
		<AutoColliders
			restitution={1}
			shape="cuboid"
			on:collisionenter={() => {
		...
		>
			<T.Mesh scale={[1, 0.1, 3]}>
				<T.BoxGeometry />
				<T.MeshStandardMaterial color="#FF2211" />
			</T.Mesh>
		</AutoColliders>
	</RigidBody>
</T.Group>

<Audio src={`/sounds/saron-${pitch}-softest.wav`} bind:play={playSoftest} bind:stop />
<Audio src={`/sounds/saron-${pitch}-soft.wav`} bind:play={playSoft} bind:stop />
<Audio src={`/sounds/saron-${pitch}-medium.wav`} bind:play={playMedium} bind:stop />
<Audio src={`/sounds/saron-${pitch}-harder.wav`} bind:play={playHarder} bind:stop />
<Audio src={`/sounds/saron-${pitch}-hardest.wav`} bind:play={playHardest} bind:stop />
